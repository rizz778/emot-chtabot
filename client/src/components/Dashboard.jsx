...
import {
  ...
  export default function Dashboard() {
    ...
    const calculateTrendDirection = () => {
      if (!data || !data.distressTrend || data.distressTrend.length < 2) return "steady";
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const response = await fetch(`https://emot-chtabot-1.onrender.com/api/profile/user`, {
            headers: {
      ...
      return (
        <div className="flex items-center justify-center h-screen">
          <div className="text-center p-6 bg-red-50 rounded-lg">
            ...
            <p className="text-gray-600">{error || "Failed to load data. Please try again later."}</p>
            <button 
    ...
    const formatDistressTrend = () => {
      if (!data || !data.distressTrend) return [];
    ...
    const formatEmotionTrend = () => {
      if (!data || !data.emotionTrend || !data.emotionCounts) return [];
    ...
    const formatEmotionRadar = () => {
      if (!data || !data.emotionCounts) return [];
    ...
    const formatEmotionPie = () => {
      if (!data || !data.emotionPercentages) return [];
    ...
    const formatSessionsTreemap = () => {
      if (!data || !data.sessionsByDistress) return [];
    ...
    const formatSessionCards = () => {
      if (!data || !data.sessionAnalytics) return [];
    ...
    const trendPercentage = () => {
      if (!data || !data.distressTrend || data.distressTrend.length < 2) return 0;
